...
{{-- @php
    if ($file && is_string($file) && !$file instanceOf Livewire\TemporaryUploadedFile) {
...
@endphp --}}
<div
    class="mb-3 grid grid-cols-4 items-center"
    x-data="{
        ...
        handleFiles() {
            ...
            if (this.isMultiple === true) {
                wire.uploadMultiple('{{ $attributes->wire('model')->value }}', this.$refs.input{{ $id }}.files, () => {
                }, () => {
                }, (event) => {
                    this.progress = event.detail.progress || 0
                });
            } else {
                wire.upload('{{ $attributes->wire('model')->value }}',  this.$refs.input{{ $id }}.files[0], () => {
                }, () => {
                }, (event) => {
                    this.progress = event.detail.progress || 0
                });
...
>
    ...
    <div class="col-span-3">
        <label for="file-{{ $id }}" class="relative block leading-tight bg-white hover:bg-gray-100 cursor-pointer inline-flex items-center transition duration-500 ease-in-out group overflow-hidden
            ...
            @if ($mode === 'profile' && $file)
                <div class="absolute inset-0">
                    <div class="relative w-full overflow-hidden shadow-inner" style="padding-bottom: 100%">
                        @if($file instanceOf Livewire\TemporaryUploadedFile && collect(['jpg', 'png', 'jpeg', 'webp'])->contains($file->getClientOriginalExtension()))
                        ...
                        @elseif($file != '' && is_string($file))
                            <img src="{{ url($file) }}" class="inset-0 w-full h-full absolute object-cover">
                    ...
                    @foreach($file as $key => $f)
                        <div class="py-3 flex {{ !$loop->last ? 'border-b border-gray-200' : '' }}">
                            <div class="w-16 mr-4 flex-shrink-0 shadow-xs rounded-lg" >
                                @if($f instanceOf Livewire\TemporaryUploadedFile && collect(['jpg', 'png', 'jpeg', 'webp'])->contains($f->getClientOriginalExtension()))
                                ...
                                @elseif($f != '' && is_string($f))
                                    <div class="relative pb-16 overflow-hidden rounded-lg border border-gray-100">
                                        <img src="{{ url($f) }}" class="w-full h-full absolute object-cover rounded-lg">
                                    </div>
                                @else
                            ...
                            <div>
                                ...
                                <button wire:key="remove-attachment-{{ $f->getFilename() }}" wire:loading.attr="disabled" type="button" x-on:click.prevent="$wire.removeUpload('{{ $attributes->wire('model')->value }}', '{{ $f->getFilename() }}')" class="text-xs text-red-500 appearance-none hover:underline">
                                    Remove
                                </button>
                            </div>
                        </div>
                    ...
                    @if($file)
                        <div class="mt-3 flex">
                            <div class="w-16 mr-4 flex-shrink-0 shadow-xs rounded-lg">
                                ...
                                @elseif($file != '' && is_string($file))
                                    <div class="relative pb-16 w-full overflow-hidden rounded-lg border border-gray-100">
                                        <img src="{{ url($file) }}" class="w-full h-full absolute object-cover rounded-lg">
                                    </div>
                                @else
                                    <div class="w-16 h-16 bg-gray-100 text-blue-500 flex items-center justify-center rounded-lg border border-gray-100">
                                        <svg class="h-12 w-12" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
