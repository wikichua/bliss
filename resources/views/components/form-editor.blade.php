    ...
    <div class="col-span-3">
        <div x-data="{
            ...
            init() {
                ...
                let quill = new Quill(this.$refs.{{ $ref }}, {
                    theme: 'snow',
                });
                ...
                quill.on('text-change', () => {
                    ...
                    wire.set('{{ $wireModel['value'] }}', this.content, true);
                });

                quill.enable({{ $attributes->get('disabled') }});
            },
        }"
        x-ref="{{ $ref }}" {{ $attributes->merge(['class' => 'block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-b-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500'])->whereDoesntStartWith('wire') }} id="{{ $id }}" {{ $attributes->whereStartsWith('wire') }} x-model="content">
        </div>
